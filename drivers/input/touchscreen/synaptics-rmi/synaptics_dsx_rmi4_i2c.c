...

	{
		...
		if( finger < sizeof( finger_info ) / sizeof( *finger_info ) )
		{
			...
			if( show_log )
			{
				touch_info->status	= !touch_info->status;
				if(state_down) {
					if(finger == 1) dt2w_2fgr = true;
					if(finger == 2) dt2w_3fgr = true;
				} else {
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	for (finger = 0; finger < 1; finger++) {
		...
		nui_rmi4_proc_fngr(rmi4_data, &currentf, finger);
	}
...
static int nui_rmi4_sensor_report(struct synaptics_rmi4_data *rmi4_data)
{
	int retval;
	unsigned char touch_count = 0;
	unsigned char intr[MAX_INTR_REGISTERS];
...
static void dt2w_irq(struct work_struct * dt2w_irq_work) {
	...
	if(touch_count > 0) {
		...
		do {
			touch_count = nui_rmi4_sensor_report(dt2w_rmi4_data);
...
static irqreturn_t nui_rmi4_irq(int irq, void *data)
{
	unsigned int i;
	...
	if (enable) {
		...
		if((no_suspend_touch)&&(scr_suspended)) {	
			retval = request_threaded_irq(rmi4_data->irq, NULL,
				nui_rmi4_irq, IRQF_NO_SUSPEND, DRIVER_NAME, rmi4_data);
			printk("ngxson: nui irq\n");
		} else {
