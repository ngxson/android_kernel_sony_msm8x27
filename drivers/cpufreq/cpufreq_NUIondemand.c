/*
 *  drivers/cpufreq/NUIcpufreq_ondemand.c
 *
 ...
 * 
 * Edited by ngxson (http://www.facebook.com/ngxson)
 */
...
//nui_batt_level
#define NUI_UP_THRESHOLD_NORMAL		(90)
#define NUI_UP_THRESHOLD_LOW		(100)
#define NUI_OPTIMAL_FREQ_SCR_OFF		(486000)
#define NUI_OPTIMAL_FREQ_LOW_BATT		(810000)
#define NUI_OPTIMAL_FREQ_NORMAL		(702000)
#define NUI_LOW_BATT	(16)
...
static void nui_early_suspend(struct early_suspend *h) {
	//ngxson
	dbs_tuners_ins.optimal_freq = NUI_OPTIMAL_FREQ_SCR_OFF;
	dbs_tuners_ins.up_threshold = NUI_UP_THRESHOLD_LOW;
}

static void nui_late_resume(struct early_suspend *h) {
	//ngxson
	if (nui_batt_level < NUI_LOW_BATT) {
		dbs_tuners_ins.optimal_freq = NUI_OPTIMAL_FREQ_LOW_BATT;
		dbs_tuners_ins.up_threshold = NUI_UP_THRESHOLD_LOW;
	} else {
		dbs_tuners_ins.optimal_freq = NUI_OPTIMAL_FREQ_NORMAL;
		dbs_tuners_ins.up_threshold = NUI_UP_THRESHOLD_NORMAL;
	}
}

static struct early_suspend nui_early_suspend_handler = {
	.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN,
	.suspend = nui_early_suspend,
	.resume = nui_late_resume,
};
...
static int __init cpufreq_gov_dbs_init(void)
{
	...
	put_cpu();
	register_early_suspend(&nui_early_suspend_handler);
	if (idle_time != -1ULL) {
...
static void __exit cpufreq_gov_dbs_exit(void)
{
	...
	unregister_early_suspend(&nui_early_suspend_handler);
	destroy_workqueue(dbs_wq);
}


MODULE_AUTHOR("ngxson <Nui luv Ice>");
MODULE_DESCRIPTION("'cpufreq_NUIondemand' - A dynamic cpufreq governor for NUI kernel");
MODULE_LICENSE("GPL");
